...
class TestHealthcheck(TestCase):
    ...
    def test_celery_available_succeeds_when_subprocess_succeeds(self, subprocess_mock):
        results = celery_available()
        assert results[0] == True

    @patch('subprocess.call', return_value=1)
    def test_celery_available_fails_when_subprocess_fails(self, subprocess_mock):
        results = celery_available()
    ...
    def test_celery_beat_available_fails_when_ping_expires(self):
        last_celery_beat_ping = datetime.now() - (2 * celery_beat_threshold())
        response = celery_beat_available()
        assert response[0] == False

    @patch('portal.views.healthcheck.db')
    def test_postgresql_available_succeeds_when_query_successful(self, db_mock):
        db_mock.engine.execute = Mock(return_value=True)
        results = postgresql_available()
        assert results[0] == True

    @patch('portal.views.healthcheck.db')
    def test_postgresql_available_fails_when_query_exception(self, db_mock):
        db_mock.engine.execute.side_effect = Mock(side_effect=Exception('Something went wrong'))
        results = postgresql_available()
        assert results[0] == False

    @patch('portal.views.healthcheck.redis')
    def test_redis_available_succeeds_when_ping_successful(self, redis_mock):
        redis_mock.from_url.return_value.ping.return_value = True
        results = redis_available()
        assert results[0] == True

    @patch('portal.views.healthcheck.redis')
    def test_redis_available_fails_when_ping_throws_exception(self, redis_mock):
