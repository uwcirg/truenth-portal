...
def celery_result(task_id):
    ...
    if isinstance(retval, dict) and retval.get("response_format"):
        return format_task_output(retval)
    else:
        return jsonify(status=task.status, result=retval)
...
@portal.route("/celery-status/<task_id>")
def celery_status(task_id):
    """Present known status details for any given celery task

    See also ``celery_result`` to obtain task output tailored to original
    request parameters.
