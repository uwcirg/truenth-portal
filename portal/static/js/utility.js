...
var intScriptId = 0, scriptsLoaded = 0, scriptsLength = 0;
function importJS(scripts, callback) {
    ...
    while (src = scripts.shift()) {
        ...
        // var len = (document.scripts).filter(function(element) {
    ...
    intScriptId = setInterval(function() {
        console.log("loaded? ", scriptsLoaded, " length: ", scriptsLength)
        if (scriptsLoaded == scriptsLength) {
            setTimeout(function() {
                callback();
            }, 50);
            clearInterval(intScriptId);
        }
    }, 10);
