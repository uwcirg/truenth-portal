...
def upgrade():
    ...
    op.create_table('contact_points',
                    ...
                    sa.Column('cp_sys', postgresql.ENUM('phone', 'fax', 'email',
                                                        'pager', 'url', 'sms', 'other', name='cp_sys'), nullable=False),
                    sa.Column('cp_use', postgresql.ENUM('home', 'work', 'temp',
                                                        'old', 'mobile', name='cp_use'), nullable=True),
    ...
    op.create_foreign_key('user_alt_phone_contact_point', 'users', 'contact_points', [
                          'alt_phone_id'], ['id'], ondelete='cascade')
    op.create_foreign_key('user_phone_contact_point', 'users', 'contact_points', [
                          'phone_id'], ['id'], ondelete='cascade')

    op.add_column(u'organizations', sa.Column(
        'phone_id', sa.Integer(), nullable=True))
    op.create_foreign_key('org_phone_contact_point', 'organizations', 'contact_points', [
                          'phone_id'], ['id'], ondelete='cascade')
    ...
    for user_id, phone in session.execute("SELECT id, phone FROM users where phone != ''"):
        ...
        cp = session.merge(cp)
        session.execute(
            'UPDATE users SET phone_id = {} where id = {}'.format(cp.id, user_id))
    ...
    for org_id, phone in session.execute("SELECT id, phone FROM organizations where phone != ''"):
        ...
        cp = session.merge(cp)
        session.execute(
            'UPDATE organizations SET phone_id = {} where id = {}'.format(cp.id, org_id))
...
def downgrade():
    ...
    for user_id, phone_id in session.execute('SELECT id, phone_id FROM users WHERE phone_id IS NOT NULL'):
        ...
        session.execute(
            "UPDATE users SET phone = '{}' where id = {}".format(phone, user_id))

    for org_id, phone_id in session.execute('SELECT id, phone_id FROM organizations WHERE phone_id IS NOT NULL'):
        phone = session.execute(
            'SELECT value FROM contact_points WHERE id = {}'.format(phone_id)).fetchone()[0]
        session.execute(
            "UPDATE organizations SET phone = '{}' where id = {}".format(phone, org_id))
