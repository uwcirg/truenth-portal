...
def upgrade():
    ...
    # migrate all existing clinicians from user.clinician_id to user_clinician rows
    results = session.execute(
        "SELECT id, clinician_id FROM users WHERE clinician_id IS NOT NULL")
    insert = text(
        """INSERT INTO user_clinicians (patient_id, clinician_id) VALUES (:patient_id, :clinician_id)""")

    for patient_id, clinician_id in results:
        session.execute(insert, {'patient_id': patient_id, 'clinician_id': clinician_id})
    session.commit()
...
def downgrade():
    ...
    op.add_column('users', sa.Column('clinician_id', sa.INTEGER(), autoincrement=False, nullable=True))
    op.create_foreign_key('users_clinician_id_fkey', 'users', 'users', ['clinician_id'], ['id'])
    op.create_index('ix_users_clinician_id', 'users', ['clinician_id'], unique=False)
    ...
    for patient_id, clinician_id in results:
        session.execute(update, {'patient_id': patient_id, 'clinician_id': clinician_id})
    session.commit()
