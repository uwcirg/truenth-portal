...
def create_missing_encounters(table_name):
    ...
    for obj_id, user_id in session.execute("SELECT id, {} FROM {} where encounter_id IS NULL".format(user_col, table_name)):
        enc = Encounter(
        ...
        session.execute('UPDATE {} SET encounter_id = {} where id = {}'.format(table_name, enc.id, obj_id))
...
def remove_generated_encounters(table_name):
    ...
    for obj_id, enc_id in session.execute("SELECT {table_name}.id, encounters.id FROM {table_name} " \
                    "JOIN encounters ON {table_name}.encounter_id = encounters.id " \
                    "WHERE encounters.status = 'finished' AND encounters.auth_method = 'staff_authenticated' " \
                    "AND encounters.start_time = '2000-01-01 01:01:01'".format(table_name=table_name)):
        session.execute('UPDATE {} SET encounter_id = NULL WHERE id = {}'.format(table_name, obj_id))
